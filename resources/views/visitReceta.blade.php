    ...
    <div class="row">
        <div class="col-4 d-flex justify-content-center" style="background-color: #76cc68">
            <div class="mt-3  fw-bolder">
                <div class="d-flex justify-content-around"><img src="{{ $info->foto }}" alt="Foto Receta" width="300" height="300"></div>
...
@auth
    ...
    @if (!$receta->likes->contains('user_id', auth()->id()))
        <form action="{{ route('receta.like', ['id' => $receta->id]) }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-primary">Like</button>
        </form>
    @else
        <p>Ya has dado like a esta receta</p>
    @endif
@endauth
        </div>
    </div>
